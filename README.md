...
Cloned challenge workspaces are not linked to an upstream remote by default. Running `git pull` will therefore report missing tracking information until you associate a remote branch:
...
After the remote is configured, `git pull` and `git push` will work as expected.
